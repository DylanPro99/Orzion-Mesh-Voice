...
class MeshManager {
  ...
  Timer? _cacheCleanupTimer; // 🧹 Timer para limpieza automática de caché
  ...
  Future<bool> initializeNetworkServices() async {
    ...
    // ⏱️ TIMEOUT: Prevenir bloqueos infinitos durante inicialización
    try {
      return await _initializeWithTimeout();
    } catch (e) {
      if (kDebugMode) {
        print('[MESH] ❌ Error durante inicialización: $e');
        print('[MESH] ℹ️ La app continuará con funcionalidad limitada');
      }
      return false;
    }
  }

  /// 🔒 Método interno con timeout para prevenir bloqueos
  Future<bool> _initializeWithTimeout() async {
    return await Future.any([
      _performInitialization(),
      Future.delayed(const Duration(seconds: 30)).then((_) {
        if (kDebugMode) {
          print('[MESH] ⏱️ TIMEOUT: Inicialización superó 30 segundos');
          print('[MESH] ℹ️ Continuando con servicios parcialmente inicializados');
        }
        _servicesInitialized = true; // Marcar como inicializado para no reintentar
        return false; // Retornar false indicando funcionalidad limitada
      }),
    ]);
  }

  /// 🚀 Lógica de inicialización extraída
  ...
  Future<void> _updateBatteryLevel() async {
    if (_battery == null || _isDisposed) return; // 🔒 Prevenir ejecución si disposed
  ...
  void _cleanOldProcessedMessages() {
    ...
    if (kDebugMode && sizeBefore > _processedMessages.length) {
      print('[MESH] 🧹 Limpieza: ${sizeBefore - _processedMessages.length} mensajes procesados eliminados');
    }
  }
  
  // 🧹 Limpieza automática de caché de mensajes para prevenir memory leaks
  void _startCacheCleanup() {
    ...
    if (kDebugMode) {
      print('[MESH] 🧹 Limpieza automática de caché activada (cada 10 minutos)');
    }
  }
  
  void _performCacheCleanup() {
    if (_isDisposed) return; // 🔒 Prevenir ejecución si disposed
    
    final cutoff = DateTime.now().subtract(const Duration(hours: 2));
    final sizeBefore = _messageCache.length;
    
    // Eliminar mensajes antiguos del caché (mantener solo últimas 2 horas)
    _messageCache.removeWhere((key, message) {
      return message.createdAt.isBefore(cutoff);
    });
    
    // Limitar tamaño máximo del caché (máximo 100 mensajes)
    if (_messageCache.length > 100) {
      final sortedMessages = _messageCache.values.toList()
        ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
      
      _messageCache.clear();
      for (var i = 0; i < 100; i++) {
        _messageCache[sortedMessages[i].messageId] = sortedMessages[i];
      }
      
      if (kDebugMode) {
        print('[MESH] 🧹 Caché limitado a 100 mensajes más recientes');
      }
    }
    
    if (kDebugMode && sizeBefore > _messageCache.length) {
      print('[MESH] 🧹 Caché limpiado: ${sizeBefore - _messageCache.length} mensajes antiguos eliminados');
      print('[MESH] 📊 Mensajes en caché: ${_messageCache.length}');
    }
  }
  ...
  void dispose() {
    if (_isDisposed) {
      if (kDebugMode) {
        print('[MESH] ⚠️ dispose() llamado múltiples veces, ignorando');
      }
      return;
    }
    ...
    if (kDebugMode) {
      print('[MESH] 🧹 MeshManager limpiado (timers cancelados, streams cerrados)');
    }
  }
