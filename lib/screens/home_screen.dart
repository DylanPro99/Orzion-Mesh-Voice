...
class _HomeScreenState extends State<HomeScreen> {
  ...
  StreamSubscription<MessageModel>? _messageSubscription; // 🔒 Subscription para limpiar
  ...
  void dispose() {
    ...
    _messageSubscription?.cancel(); // Cancelar subscription de mensajes
    
    if (mounted) {
      debugPrint('🧹 HomeScreen: Limpieza completada');
    }
  ...
  Future<void> _initializeMesh() async {
    ...
    try {
      ...
      // 🔒 Asegurar que StorageService esté inicializado (debería estarlo desde main.dart)
      try {
        final storageService = StorageService();
        await storageService.initialize();
        debugPrint('✅ StorageService verificado');
      } catch (e) {
        debugPrint('⚠️ Error verificando StorageService: $e');
      }
  ...
  void _setupMessageListener() {
    // 🔒 Guardar subscription para poder cancelarla en dispose
    _messageSubscription = _meshManager.incomingMessages.listen(
      (message) {
        if (mounted) {
          setState(() {});
      ...
      cancelOnError: false, // No cancelar el stream si hay un error
    );
  }
