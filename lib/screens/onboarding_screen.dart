...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _completeOnboarding() async {
    ...
    try {
      // Intentar inicializar servicios, pero continuar incluso si falla
      ...
      if (mounted) {
        ...
        // Mostrar mensaje informativo si se denegó permiso de notificaciones
        if (!hasFullFunctionality) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('⚠️ Sin permiso de notificaciones: La app solo funcionará en primer plano.\n\nPuedes activarlo en Configuración > Permisos si cambias de opinión.'),
      ...
      if (kDebugMode) {
        print('[Onboarding] ⚠️ Error durante inicialización: $e');
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('⚠️ Iniciado con funcionalidad limitada. Por favor, verifica permisos.'),
            backgroundColor: Colors.orange,
            duration: Duration(seconds: 4),
          ),
        );
        
        // Continuar a la pantalla principal incluso si la inicialización falla
        // Los servicios se pueden reintentar posteriormente
        Navigator.pushReplacementNamed(context, '/home');
      }
    }
  }
